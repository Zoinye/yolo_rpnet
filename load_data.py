...
class labelFpsDataLoader1(Dataset):
    ...
    def __getitem__(self, index):
        ...
        new_labels = [(leftUp[0] + rightDown[0]) / (2 * ori_w), (leftUp[1] + rightDown[1]) / (2 * ori_h),
                      (rightDown[0] - leftUp[0]) / ori_w, (rightDown[1] - leftUp[1]) / ori_h] # xywh

        return resizedImage, new_labels, lbl, img_name
class labelFpsDataLoader(Dataset):
    def __init__(self, img_name, img_size, is_transform=None):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(img_name, list):
            img_name = img_name[0]  # 如果是列表，获取第一张图片的路径
        img = cv2.imread(img_name)

        # resized_image = cv2.resize(img, (self.img_size,self.img_size))
        # resized_image = np.transpose(resized_image, (2, 0, 1)).astype('float32') / 255.0
        ...
        return new_labels, lbl, img_name
