...
def roi_pooling_ims(input, rois, size=(7, 7), spatial_scale=1.0):
    ...
    for i in range(rois.size(0)):
        ...
        im = input[roi[0]:roi[0] + 1, :, y1:y2 + 1, x1:x2 + 1]
        if im.numel() == 0:
            continue
        try:
...
class CombinedModel(nn.Module):
    ...
    def forward(self, x, detection_targets,YI):
        ...
        roi1 = roi_pooling_ims(x1, boxNew.matmul(p1), size=(16, 8))
        roi2 = roi_pooling_ims(x2, boxNew.matmul(p2), size=(16, 8))
        roi3 = roi_pooling_ims(x3, boxNew.matmul(p3), size=(16, 8))

        # Concatenate ROIs and pass through classifier
        rois = torch.cat((roi1, roi2, roi3), dim=1)
