...
from models.rpnet import CombinedModel, LicensePlateClassifier
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):
        ...
        self.grid = [torch.empty(0) for _ in
                     range(self.nl)]  # init grid(1,3,4,4,2) 单批次，每个网格单元的锚点数量，特征图的高度和宽度（网格单元的数量），网格单元的 x 和 y 坐标
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                if isinstance(self, Segment):  # (boxes + masks) #分割任务
                    xy, wh, conf, mask = x[i].split((2, 2, self.nc + 1, self.no - self.nc - 5), 4)
                    xy = (xy.sigmoid() * 2 + self.grid[i]) * self.stride[i]  # xy
                    wh = (wh.sigmoid() * 2) ** 2 * self.anchor_grid[i]  # wh
                    y = torch.cat((xy, wh, conf.sigmoid(), mask), 4)
                else:  # Detect (boxes only)
                    xy, wh, conf = x[i].sigmoid().split((2, 2, self.nc + 1), 4)  # x[i][1, 3, 4, 4, 8]
...
def parse_model(d, ch):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            c1, c2 = ch[f], args[0]  # 输入总的通道，anchor
